...
function demoAnalysis(question: string, topic: Topic, locale: Locale, sources: ApprovedSource[]): GeneratedAnalysis {
  ...
  const topicCopy: Record<Topic, { zh: [string, string, string]; en: [string, string, string] }> = {
    ...
    other: { zh: ['先判断真正需要解决的是哪一类问题', '这个问题跨越了常见分类，因此更需要先分离事实、欲望、可控范围与关键假设，再选择最有解释力的公开思想框架。分类只是入口，不应该替代对具体处境的判断。', '当问题同时包含多个维度时，一次能暴露关键假设的小行动，比强行把它塞进某个固定类别更可靠。'], en: ['First identify what kind of problem this really is', 'This question crosses the usual categories, so start by separating facts, desires, controllable factors, and key assumptions before choosing the most explanatory public frameworks. A category is only an entry point, not a substitute for judgment.', 'When a question spans several dimensions, a small action that exposes the key assumption is more reliable than forcing it into one predefined category.'] },
  };
