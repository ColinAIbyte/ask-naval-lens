...
export const approvedSources: ApprovedSource[] = [
  {
    ...
    tags: ['wealth', 'freedom', 'ownership', 'leverage', 'equity', 'code', 'media', 'business', 'audience'],
  ...
  },
  {
    ...
    tags: ['career', 'curiosity', 'skills', 'strengths', 'specific knowledge', 'learning', 'product', 'founder'],
  ...
  },
  {
    ...
    tags: ['career', 'apprenticeship', 'creative', 'technical', 'automation', 'skills', 'mentor', 'team'],
  ...
  },
  {
    ...
    sourceType: 'podcast_transcript',
    topics: ['wealth', 'career', 'entrepreneurship', 'decision_making'],
    tags: ['accountability', 'risk', 'reputation', 'credibility', 'ownership', 'founder', 'business', 'responsibility'],
  ...
  },
  {
    ...
    tags: ['career', 'job', 'quit', 'runway', 'time', 'founder', 'experience', 'judgment', 'autonomy'],
  ...
  },
  {
    ...
    tags: ['decision', 'experiment', 'action', 'feedback', 'risk', 'uncertainty', 'quit', 'opportunity', 'choice'],
  ...
  },
  {
    ...
    tags: ['decision', 'judgment', 'long term', 'consequences', 'opportunity', 'experience', 'prestige', 'risk'],
  ...
  },
  {
    ...
    sourceType: 'podcast_transcript',
    topics: ['wealth', 'career', 'entrepreneurship'],
    tags: ['salary', 'time', 'income', 'freedom', 'equity', 'ownership', 'invest', 'business'],
  ...
  },
  {
    ...
    sourceType: 'podcast_transcript',
    topics: ['wealth', 'career', 'entrepreneurship', 'decision_making'],
    tags: ['salary', 'runway', 'savings', 'freedom', 'lifestyle', 'quit', 'risk', 'optionality'],
  ...
  },
  {
    ...
    tags: ['happiness', 'anxiety', 'desire', 'achievement', 'promotion', 'comparison', 'ambition', 'peace', 'success'],
  ...
  },
  {
    ...
    tags: ['happiness', 'success', 'ambition', 'comparison', 'meaning', 'indirect', 'money', 'purpose'],
...
export function retrieveSources(question: string, topic: Topic | null, limit = 6): ApprovedSource[] {
  ...
  const tokens = new Set(haystack.split(/\s+/).filter((token) => token.length > 3 && !stopWords.has(token)));
  const scored = approvedSources.map((source, index) => {
    let score = topic && source.topics.includes(topic) ? 8 : 0;
    for (const tag of source.tags) {
      const normalizedTag = normalize(tag);
      if (haystack.includes(normalizedTag)) score += normalizedTag.includes(' ') ? 7 : 5;
    }
    const searchable = normalize(`${source.title} ${source.summary}`);
