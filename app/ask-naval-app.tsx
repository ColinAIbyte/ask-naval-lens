...
const copy = {
  zh: {
    ...
    label: '你现在想理清什么？', placeholder: '我在一家稳定的公司工作了五年，但越来越想辞职做自己的产品。我有一年的生活费和几个潜在客户，却担心在经济疲软时辞职风险太大。我应该如何思考这个决定？', topics: ['财富', '职业', '创业', '决策', '幸福', '其他'],
    ...
    howTrustTitle: '每次分析都遵守', howTrustItems: ['人工整理的公开来源库', '结构化输出', '不冒充本人'],
...
export default function AskNavalApp({ initialLocale }: { initialLocale: Locale }) {
  ...
  useEffect(() => {
    ...
    if (sharedQuestion && sharedQuestion.length <= 3000) setQuestion(sharedQuestion);
  ...
  async function analyze() {
    ...
    track('analysis_submitted', { topic: selectedTopic ?? 'other', locale });
    try {
      ...
      track('analysis_completed', { topic: selectedTopic ?? 'other', locale });
      ...
      setError(message); setStatus('error'); track('analysis_failed', { topic: selectedTopic ?? 'other', locale });
    ...
    <main className="site-shell">
      ...
      <section id="top" className="hero-grid">
        <div className="hero-copy"><p className="eyebrow"><span />{t.eyebrow}</p><h1>{t.titleA}<br />{locale === 'zh' ? <span className="hero-emphasis">{t.titleB}</span> : <em>{t.titleB}</em>}</h1><p className="subtitle">{t.subtitle}</p><p className="hero-disclaimer">{t.disclaimer}</p>
          <div className="ask-card"><label htmlFor="question">{t.label}</label><textarea id="question" value={question} onChange={(event) => setQuestion(event.target.value)} placeholder={t.placeholder} minLength={30} maxLength={3000} disabled={status === 'loading'} />
      ...
      <section className="examples-section" aria-labelledby="examples-title"><p id="examples-title">{t.examples}</p><div className="examples-list">{t.exampleItems.map((item, index) => <button key={item} onClick={() => { setQuestion(item); setTopic(topicKeys[[2, 3, 0][index]]); track('example_selected', { example_id: index, locale }); }} type="button"><span>{item}</span><span aria-hidden="true">→</span></button>)}</div></section>
      ...
      {analysis && analysisId && resultUrl && <div ref={answerRef}><AnalysisResult locale={locale} analysis={analysis} question={question} topicLabel={topic ? t.topics[topicKeys.indexOf(topic)] : t.topics[5]} analysisId={analysisId} resultUrl={resultUrl} onAskAnother={resetQuestion} onFollowUp={askFollowUp} /></div>}
