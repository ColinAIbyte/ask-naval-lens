...
const contactEmails: Record<Locale, string> = {
  zh: '2685897@qq.com',
  en: 'fancifulman2008@gmail.com',
};

const copy = {
  zh: {
    navHow: '它如何工作', navPricing: '体验与价格', signIn: '使用 ChatGPT 登录', signOut: '退出', account: '已登录账号', guest: '无需登录', contact: '合作联系', eyebrow: '不是替你决定，而是帮你看清',
    titleA: '把难做的决定，', titleB: '看清楚，再行动。', subtitle: '写下你的真实处境。系统会从 Naval Ravikant 的公开思想中匹配相关框架，解释为什么适用，并给出 3 个可验证的下一步。',
    label: '写下你正在反复权衡的真实问题', placeholder: '例如：我现在应该辞职创业吗？', topics: ['财富', '职业', '创业', '决策', '幸福', '其他'],
    topicHint: '选择最接近的主题（可选）', inputGuide: '建议写清：你的处境、正在比较的选项，以及最担心什么。', generalTopic: '综合分析',
    cta: '用 Naval 的思想分析', analyzing: '正在从 Naval 的思想框架中分析……', free: (n: number) => n > 0 ? `本周免费分析还剩 ${n} 次` : '本周免费额度已用完', credits: (n: number) => `剩余 ${n} 次付费分析`,
    ...
    lens: '你会得到一份怎样的分析', lensItems: ['你真正要决定的问题', '1–2 个最相关思想框架', '为什么适合你的处境', '3 个可验证的下一步', '每个框架的原始出处'],
    quote: '不是替你做决定，而是帮你把决定看得更清楚。', disclaimer: '本工具独立制作，与 Naval Ravikant 本人无官方关联。',
    shortQuestion: '请至少输入 8 个字符的问题；补充真实背景会得到更具体的分析。', topicRequired: '主题为选填项。', genericError: '暂时无法完成分析，请稍后重试。',
    ...
    pricingEyebrow: '简单、透明的早期价格', pricingTitle: '无需账号，每周先用 3 次。', pricingBody: '中文体验无需 ChatGPT 账号。每周三次完整免费分析；付费功能开放后也不会把 ChatGPT 账号作为购买前提。',
    ...
    close: '暂时不用', paymentUnavailable: '中文版付费功能正在准备中。正式开放后不要求 ChatGPT 账号；目前请先使用每周免费额度。',
    howEyebrow: '不是泛泛聊天', howTitle: '从一团纠结，到一个可以验证的下一步。',
    howTrustTitle: '这份分析如何保证可信', howTrustItems: ['只使用可核验的 Naval 公开内容', '思想框架与原始出处一一对应', '不扮演、不代表 Naval 本人'],
    howSteps: [['说清处境', '写下你面对的选择、已经知道的事实，以及真正让你犹豫的地方。'], ['匹配框架', '默认只选 1–2 个最相关框架；确有必要时才加入第 3 个，并附上原始出处。'], ['设计验证', '把判断变成三个有完成时间和建议验证信号的行动，不让答案停在“想明白了”。']],
    howProof: [['真实选择', '例：该不该辞职创业？'], ['1–2 个框架', '公开出处'], ['今天', '7 天内', '30 天内']],
  ...
  en: {
    navHow: 'How it works', navPricing: 'Pricing', signIn: 'Sign in with ChatGPT', signOut: 'Sign out', account: 'Signed-in account', guest: 'No sign-in required', contact: 'Work with me', eyebrow: 'Clarity starts with a better frame',
    ...
    lens: "What's inside your analysis", lensItems: ['Naval-inspired perspective', '1–2 relevant mental models', 'Why they fit your situation', '3 concrete next steps', 'Public sources'],
    ...
    howSteps: [['Ask honestly', 'Turn a vague worry into the real-world choice you actually need to answer.'], ['Match the frame', 'Use the 1–2 most relevant mental models by default; add a third only when it clearly earns its place.'], ['Act clearly', 'Turn the judgment into three time-bound next steps with suggested validation signals.']],
    howProof: [['A real choice', 'e.g. Should I quit to build?'], ['1–2 frameworks', 'Public sources'], ['Today', 'Within 7 days', 'Within 30 days']],
...
export default function AskNavalApp({ initialLocale, initialQuestion = '', auth }: { initialLocale: Locale; initialQuestion?: string; auth: AuthState }) {
  ...
  async function analyze() {
    ...
    if (question.trim().length < minimumQuestionLength) { setError(t.shortQuestion); return; }
    ...
    } finally { isSubmittingRef.current = false; setStatus((current) => current === 'error' ? 'error' : 'idle'); }
  }

  async function startCheckout() {
    setPaymentMessage(''); track('checkout_started', { plan_id: 'starter_30', locale });
    if (locale === 'zh') {
      setPaymentMessage(t.paymentUnavailable);
      track('checkout_unavailable', { reason: 'zh_guest_payment_pending', locale });
      return;
    }
    try {
  ...
  return (
    <main className={`site-shell locale-${locale}`}>
      <nav className="nav-wrap" aria-label={locale === 'zh' ? '主要导航' : 'Primary navigation'}>
        ...
        <div className="nav-links"><a className="nav-how" href="#how">{t.navHow}</a><a className="nav-pricing" href="#pricing">{t.navPricing}</a>{auth.user ? <div className="nav-account" title={`${t.account}: ${auth.user.email}`} aria-label={`${t.account}: ${auth.user.email}`}><span className="nav-account-dot" aria-hidden="true" /><span className="nav-account-name">{auth.user.displayName}</span><a className="nav-signout" href={auth.signOutUrl}>{t.signOut}</a></div> : locale === 'zh' ? <span className="nav-guest"><span aria-hidden="true">✓</span>{t.guest}</span> : <a className="nav-signin" href={auth.signInUrl} onClick={() => track('signin_clicked', { locale })}>{t.signIn}</a>}
